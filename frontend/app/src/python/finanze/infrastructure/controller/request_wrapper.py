    ...
    def __init__(self, method, path, body, headers, args):
            ...
            for k, v in body.items():
                if isinstance(v, dict) and v.get("_type") == "file":
                    ...
                    if isinstance(content, dict):
                        ...
                        elif "buffer" in content:
                            content = content.get("buffer")
                        elif all(str(key).isdigit() for key in content.keys()):
                            content = [
                                content[key]
                                for key in sorted(content.keys(), key=lambda x: int(x))
                            ]
        ...
        if not self.content_type:
            if self.files:
                ...
                self.form
                and "data" in self.form
                and isinstance(self.form.get("data"), str)
            ):
                self.content_type = "multipart/form-data"
            else:
