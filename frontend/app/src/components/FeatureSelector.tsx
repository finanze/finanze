...
import {BarChart, CheckCircle, History, Receipt, Repeat, Send} from "lucide-react"
      ...
      <CardContent>
        <div className="space-y-4">
          {availableFeatures.length > 1 && (
            <Button variant="outline" className="w-full" onClick={selectAllFeatures}>
          ...
          <div className="grid grid-cols-2 gap-3">
            {availableFeatures.map((feature) => (
              <motion.div key={feature} whileHover={{scale: 1.02}} whileTap={{scale: 0.98}}>
                <Button
                  variant={selectedFeatures.includes(feature) ? "default" : "outline"}
                  className="w-full h-20 flex flex-col justify-center items-center gap-2"
                  onClick={() => toggleFeature(feature)}
                >
                  {selectedFeatures.includes(feature) && <CheckCircle className="absolute top-2 right-2 h-4 w-4"/>}
          ...
          <Button className="w-full mt-4" disabled={selectedFeatures.length === 0 || isLoading} onClick={handleSubmit}>
