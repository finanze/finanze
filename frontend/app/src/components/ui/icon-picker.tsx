  ...
  (
    ...
    const categorizedIcons = useMemo(() => {
      if (!categorized || search.trim() !== "") {
        return [{ name: t.iconPicker.allIcons, icons: filteredIcons }]
      }
      ...
      filteredIcons.forEach(icon => {
        if (icon.categories && icon.categories.length > 0) {
          ...
          const category = t.iconPicker.other
          if (!categories.has(category)) {
            categories.set(category, [])
          }
          categories.get(category)!.push(icon)
    ...
    const categoryButtons = useMemo(() => {
      ...
      return categorizedIcons.map(category => (
        ...
        >
          {(t.iconPicker.categories as Record<string, string>)[category.name] ??
            category.name}
        </Button>
      ))
              ...
              return (
                ...
                >
                  <h3 className="font-medium text-sm capitalize">
                    {(t.iconPicker.categories as Record<string, string>)[
    ...
    return (
      ...
      >
        <PopoverTrigger ref={ref} asChild {...props}>
          {children || (
            <Button
              variant="outline"
              aria-label={
                (value || selectedIcon) ?? t.iconPicker.triggerPlaceholder
              }
            >
              {value || selectedIcon ? (
                ...
                <span className="text-muted-foreground">
                  {triggerPlaceholder ?? t.iconPicker.triggerPlaceholder}
                </span>
              )}
            </Button>
          )}
        ...
        <PopoverContent className="w-64 p-2">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <Icon name={"trash-2" as IconName} className="w-4 h-4" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>{t.iconPicker.removeIcon}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          ...
          {searchable && (
            <Input
              placeholder={searchPlaceholder ?? t.iconPicker.searchPlaceholder}
