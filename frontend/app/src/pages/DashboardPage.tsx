...
export default function DashboardPage() {
    ...
    const [assetDistributionCardSmall, setAssetDistributionCardSmall] = useState(false);
    ...
    useEffect(() => {
        ...
        const checkScreenSize = () => {
            if (assetDistributionCardElement) {
                setAssetDistributionCardSmall(assetDistributionCardElement.clientWidth < 600);
            }
        };
            ...
            if (entityPosition.investments) {
                ...
                if (
                    ...
                    entityPosition.investments.stocks.details.forEach(stock => {
                        totalInvested += stock.initial_investment || (stock.shares && stock.average_buy_price ? stock.shares * stock.average_buy_price : stock.market_value || 0);
                    });
                }
    ...
    const getOngoingProjects = () => {
        ...
        Object.values(positionsData.positions).forEach((entityPosition) => {
            if (entityPosition.investments) {
                if (
                    ...
                    entityPosition.investments.deposits.details.forEach(
                        (deposit) => {
                            if (deposit.maturity) {
                                projects.push({
                ...
                if (
                    ...
                    entityPosition.investments.real_state_cf.details.forEach(
                        (project) => {
                            if (project.maturity) {
                                projects.push({
                                    name: project.name,
                ...
                if (
                    ...
                    entityPosition.investments.factoring.details.forEach(
                        (factoring) => {
        ...
        return projects
            .sort(
                (a, b) =>
                    new Date(a.maturity).getTime() -
                    new Date(b.maturity).getTime()
            )
            .slice(0, 12)
    }
        ...
        Object.values(positionsData.positions).forEach((entityPosition) => {
            if (entityPosition.investments) {
                if (
                    ...
                    entityPosition.investments.stocks.details.forEach(
                        (stock) => {
                            ...
                            allPositionsRaw.push({
                                ...
                                change:
                                    ((value /
                                        (stock.initial_investment || value || 1)) -
                                        1) *
                                    100,
                                entity:
                                    entityPosition.entity?.name,
                ...
                if (
                    ...
                    entityPosition.investments.funds.details.forEach((fund) => {
                        ...
                        allPositionsRaw.push({
                            ...
                            type: 'FUND',
                            change:
                                ((value / (fund.initial_investment || value || 1)) -
                                    1) *
                                100,
    ...
    const fundPortfolioColorMap = useMemo(() => {
        const uniqueNames = Array.from(new Set(fundItems.filter(item => item.portfolioName).map(item => item.portfolioName as string)));
        const colors = [
            'bg-sky-500', 'bg-blue-500', 'bg-indigo-500', 'bg-violet-500', 'bg-purple-500', 'bg-fuchsia-500', 'bg-pink-500', 'bg-rose-500',
            'bg-cyan-500', 'bg-teal-500', 'bg-emerald-500', 'bg-green-500', 'bg-lime-500', 'bg-yellow-500', 'bg-amber-500', 'bg-orange-500',
        ];
    ...
    const ITEM_FUND_COLORS = [
        'bg-sky-500', 'bg-sky-400', 'bg-sky-300', 'bg-sky-600', 'bg-sky-700',
        'bg-cyan-500', 'bg-cyan-400',
        'bg-green-500', 'bg-green-400', 'bg-teal-500', 'bg-teal-400',
        'bg-emerald-500', 'bg-emerald-400', 'bg-lime-500', 'bg-lime-400',
        'bg-yellow-500', 'bg-yellow-400'
    ];
    const ITEM_STOCK_COLORS = [
        'bg-violet-500', 'bg-violet-400', 'bg-violet-300', 'bg-violet-600', 'bg-violet-700',
        'bg-purple-500', 'bg-purple-400',
        'bg-pink-500', 'bg-pink-400', 'bg-rose-500', 'bg-rose-400',
    ...
    const getItemColorByIndex = (index: number, type: 'FUND' | 'STOCK_ETF') => {
        const colors = type === 'FUND' ? shuffledFundItemColors : shuffledStockItemColors;
    ...
    const CustomLegend = (props: any) => {
        const { payload } = props;
        return (
            <ul className="space-y-1.5 text-xs scrollbar-thin pr-0.5"> {/* Removed max-h and overflow-y */}
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className="capitalize truncate flex-grow min-w-0">
                                {(t.enums.productType as any)[assetType] || assetType.toLowerCase().replace(/_/g, ' ')}
                            </span>
                            ...
                            <Card>
                                ...
                                <CardContent ref={assetDistributionCardRef}>
                                    {assetDistribution.length > 0 ? (
                                        <div className={assetDistributionCardSmall ? "h-[450px]" : "h-[300px]"}>
                                                ...
                                                <PieChart>
                                                    ...
                                                    >
                                                        {assetDistribution.map((entry, index) => (
                                                            <Cell key={`cell-${index}`} fill={getPieSliceColorForAssetType(entry.type)} />
                                                        ))}
                                                    </Pie>
                                                    <Tooltip formatter={(value: number, name: string, props: any) => [`${formatCurrency(value, locale, settings?.mainCurrency)} (${props.payload.percentage}%)`, t.enums && t.enums.productType && (t.enums.productType as any)[name] ? (t.enums.productType as any)[name] : name.toLowerCase().replace(/_/g, ' ')]} />
                                                    <Legend
                                                        layout="vertical"
                                                        verticalAlign={assetDistributionCardSmall ? 'bottom' : 'middle'}
                                                        align={assetDistributionCardSmall ? 'center' : 'right'}
                                                        wrapperStyle={
                                                            assetDistributionCardSmall
                                                                ? { maxHeight: '200px', overflowY: 'auto', paddingTop: '25px', width: '95%', margin: '0 auto' }
                                                                : { paddingRight: '20px', maxHeight: '260px', overflowY: 'auto' }
                                                        }
                                ...
                                <CardContent>
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        {totalInvestedAmount > 0 && (() => {
                                            const percentageValue = ((totalAssets - totalInvestedAmount) / totalInvestedAmount) * 100;
                                            const sign = percentageValue >= 0 ? '+' : '-';
                                            return (
                                                <p className={`text-xl font-medium ${percentageValue === 0 ? 'text-gray-500 dark:text-gray-400' : (percentageValue > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400')}`}>
                                                    {sign}{formatPercentage(Math.abs(percentageValue), locale)}
                                                </p>
                                            );
                                        })()}
                                    </div>
                                    <p className="text-sm text-muted-foreground mt-1">
                                        {t.dashboard.investedAmount} {formatCurrency(
                                            totalInvestedAmount,
                                            locale,
                                            settings?.mainCurrency
                                        )}
                                                            ...
                                                            <CardContent className="p-4 flex flex-col justify-between h-full">
                                                                <div>
                                                                    <div className="flex justify-between items-start mb-3">
                                                                            ...
                                                                            <div className="flex items-center text-xs text-muted-foreground mt-1">
                                                                                ...
                                                                                <div className="flex items-center">
                                                                                    ...
                                                                                    <span className="ml-1 capitalize">
                                                                                        {t.enums && t.enums.productType && (t.enums.productType as any)[project.type]
                                                                                            ? (t.enums.productType as any)[project.type]
                                                                        ...
                                                                        <Badge
                                                                            variant="outline"
                                                                            className={`flex-shrink-0 h-auto px-2 py-1 text-center text-xs whitespace-nowrap ${status.isDelayed
                                    ...
                                    {fundItems.length > 0 || stockItems.length > 0 ? (
                                        <div className="flex flex-grow space-x-3 overflow-hidden">
                                            <div className="flex-grow space-y-2 overflow-y-auto scrollbar-thin pr-2">
                                                {fundItems.length > 0 ? (<div className='pb-2'>
                                                    ...
                                                    {fundItems.map((item, index) => (
                                                        <div key={item.id} className="flex items-stretch space-x-2 py-1.5 border-b border-border last:border-b-0">
                                                            <div className={`flex-shrink-0 w-1 rounded-sm ${getItemColorByIndex(index, 'FUND')}`}></div>
                                                            <div className="flex-grow min-w-0">
                                                                {/* Line 1: Name, Portfolio Badge, Value */}
                                                                <div className="flex justify-between items-center">
                                                                    <div className="flex items-center flex-grow min-w-0 mr-2">
                                                                        <span className="font-medium truncate text-base" title={item.name}>{item.name}</span>
                                                                        {item.portfolioName && (
                                                                            <Badge
                                                                            ...
                                                                            >
                                                                                <span className="truncate max-w-[120px]">{item.portfolioName}</span>
                                                                            </Badge>
                                                                        )}
                                                                    </div>
                                                                    <span className="font-semibold whitespace-nowrap text-sm">{item.formattedValue}</span>
                                                                </div>

                                                                {/* Line 2: Change % */}
                                                                <div className="flex justify-end items-center text-muted-foreground text-xs mt-0.5">
                                                                    <span className={`whitespace-nowrap ${item.change >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                                                                ...
                                                                <div className="flex justify-between items-center text-muted-foreground text-xs mt-0.5">
                                                                    <Badge variant="outline" className="py-0.5 px-1.5 text-[10px] leading-tight">{item.entity}</Badge>
                                                                    <span className="text-[10px] whitespace-nowrap">{formatPercentage(item.percentageOfTotalVariableRent, locale)} {t.dashboard.ofPortfolioShareShort}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ))}
                                                ...
                                                {stockItems.length > 0 ? (<div>
                                                    <h3 className="text-sm font-semibold mb-1.5 text-muted-foreground sticky top-0 bg-card z-10 py-1">
                                                        {t.dashboard.stocksAndFunds.stocksEtfs}
                                                    </h3>
                                                    {stockItems.map((item, index) => (
                                                        <div key={item.id} className="flex items-stretch space-x-2 py-1.5 border-b border-border last:border-b-0">
                                                            <div className={`flex-shrink-0 w-1 rounded-sm ${getItemColorByIndex(index, 'STOCK_ETF')}`}></div>
                                                            <div className="flex-grow min-w-0">
                                                                <div className="flex justify-between items-center">
                                                                    <span className="font-medium truncate flex-1 mr-2 text-base" title={item.name}>{item.name}</span>
                                                                    <span className="font-semibold whitespace-nowrap text-sm">{item.formattedValue}</span>
                                                                </div>
                                                                <div className="flex justify-between items-center text-muted-foreground text-xs">
                                                                    {item.symbol ? (
                                                                        <span className="truncate" title={item.symbol}>{item.symbol}</span>
                                                                    ) : (
                                                                        <span>&nbsp;</span>
                                                                    )}
                                                                    <span className={`whitespace-nowrap ${item.change >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                                                        ...
                                                        {fundItems.map((item, index) => (
                                                            item.percentageOfTotalVariableRent > 0 && (
                                                                <div
                                                                    ...
                                                                    style={{ height: `${Math.max(item.percentageOfTotalVariableRent, 1)}%` }}
                                                                ></div>
                                                            )
                                                        ))}
                                                        {fundItems.length > 0 && stockItems.length > 0 && stockItems.some(si => si.percentageOfTotalVariableRent > 0) && fundItems.some(fi => fi.percentageOfTotalVariableRent > 0) && (
                                ...
                                <CardContent className="flex-grow overflow-y-auto scrollbar-thin min-h-[350px] max-h-[650px]">
                                    {Object.keys(recentTransactions).length > 0 ? (
                                        <ul className="space-y-0">
                                            {Object.entries(recentTransactions).map(([date, txsOnDate]) => (
                                                <li key={date} className="py-2">
                                                    ...
                                                    <ul className="space-y-0 pr-4">
                                                        {(txsOnDate as any[]).map((tx: any, index: number) => (
                                                            <li
                                                            ...
                                                            >
                                                                <div className="flex items-center flex-grow min-w-0">
                                                                    ...
                                                                    <div className="flex-grow min-w-0">
                                                                        ...
                                                                        <div className="flex items-center space-x-1.5 text-xs text-muted-foreground mt-0.5">
                                                                            <Badge variant="outline" className="py-0.5 px-1.5 text-[10px] leading-tight">{tx.entity}</Badge>
                                                                            {tx.product_type && (
                                                                                <Badge variant="secondary" className="py-0.5 px-1.5 text-[10px] leading-tight">
                                                                                    {t.enums && t.enums.productType && (t.enums.productType as any)[tx.product_type]
                                                                                        ? (t.enums.productType as any)[tx.product_type]
                                                                                        : tx.product_type.toLowerCase().replace(/_/g, ' ')}
                                                                                </Badge>
                                                                            )}
                                                                        </div>
                                                                    </div>
                                                                ...
                                                                <div className="text-right flex-shrink-0 pl-2">
                                                                    <p
                                                                        className={`text-sm font-semibold ${tx.displayType === 'income'
