...
interface HistoricDisplayItem {
  ...
  grossInterestRateFormatted: string
  projectType: string
  ...
  const historicDisplayItems = useMemo<HistoricDisplayItem[]>(() => {
    ...
    return historicEntries
      ...
      .map(entry => {
        ...
        const grossInterestRateFormatted =
          entry.gross_interest_rate != null
            ? `${(entry.gross_interest_rate * 100).toFixed(2)}%`
            : notAvailableLabel

        const formatDateSafe = (value?: string | null) =>
          value ? formatDate(value, locale) : notAvailableLabel

        const projectType = translateProjectType(entry.type)
        ...
        >
          ...
          {historicError ? (
            ...
            <Card className="p-10 text-center text-sm text-gray-600 dark:text-gray-400">
              ...
              ).replace("{type}", t.common.factoring)}
            </Card>
          ) : (
