...
interface AppContextType {
    ...
    scrape: (entity: Entity, features: Feature[], options?: object) => Promise<void>
    ...
    clearPinError: () => void
    startExternalLogin: (entity?: Entity, credentials?: Record<string, string>) => Promise<void>
...
export function AppProvider({ children }: { children: ReactNode }) {
    ...
    const [currentAction, setCurrentAction] = useState<"login" | "scrape" | null>(null)
    const [storedCredentials, setStoredCredentials] = useState<Record<string, string> | null>(null)
    const [toast, setToast] = useState<{ message: string; type: "success" | "error" | "warning" | null } | null>(null)
    const [view, setView] = useState<"entities" | "login" | "features" | "external-login">("entities")
    ...
    const handleLoginError = (code: string) => {
        const errorMessage = t.errors[code as keyof typeof t.errors] || t.common.loginError
        showToast(errorMessage, "error")
    }

    const handleScrapeError = (code: string) => {
        const errorMessage = t.errors[code as keyof typeof t.errors] || t.common.fetchError
        showToast(errorMessage, "error")
    }
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && window.ipcAPI) {
            ...
            const cleanupListener = window.ipcAPI.onCompletedExternalLogin((id, result) => {
                ...
                if (result.success) {
                    ...
                    if (scrapeManualLogin.current.active) {
                        ...
                        const allCredentialsProvided = Object.keys(visibleCredentials).every((key) => result.credentials[key])
    ...
    // Handle completion of manual login during scraping
    const handleScrapeManualLoginCompletion = async (credentials: Record<string, string>) => {
        ...
        try {
            ...
            if (loginResponse.code === LoginResultCode.CREATED || loginResponse.code === LoginResultCode.RESUMED) {
    ...
    const startExternalLogin = async (entityOverride?: Entity, credentials?: Record<string, string>) => {
        ...
        try {
            ...
            const result = await window.ipcAPI.requestExternalLogin(entityToUse.id, { credentials })
    ...
    const scrape = async (entity: Entity, features: Feature[], options: object = {},) => {
        if (!entity) return
    ...
    useEffect(() => {
        setActiveEntities(entities.filter(entity => entity.status !== EntityStatus.DISCONNECTED))
    }, [entities])
