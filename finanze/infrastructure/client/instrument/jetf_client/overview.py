...
from infrastructure.client.instrument.jetf_client.helpers import (
    ASSET_CLASSES,
    EXCHANGES,
    INSTRUMENTS,
    REGIONS,
...
async def load_raw_overview(
    ...
    async with httpx.AsyncClient(headers={"User-Agent": USER_AGENT}) as client:
        html_response = await client.get(f"{BASE_URL}?search=ETFS")
    ...
    if enrich:
        for enrichment_name, enrichments in {
            ...
            for enrichment, value in cast(dict, enrichments).items():
                ...
                for row in await load_raw_overview(**kwargs):  # type: ignore[arg-type]
