...
from infrastructure.client.instrument.jetf_client.live_quote import load_live_quote
from infrastructure.client.instrument.jetf_client.jetf_types import Quote
...
async def _fetch_ajax_data(
    client: httpx.AsyncClient, isin: str, endpoint: str, element_id: str
) -> Optional[str]:
    """
    Fetch expanded data via AJAX request (for countries/sectors load more).

    Args:
        client: httpx AsyncClient with cookies from main page
    ...
    try:
        resp = await client.get(ajax_url, headers=ajax_headers)
...
async def get_etf_overview(
    ...
    async with httpx.AsyncClient() as client:
        ...
        for row in holdings_rows:
            ...
            if name_elem and pct_elem:
                ...
                if holding_name and pct is not None:
                    top_holdings.append(
                        HoldingItem(
                            name=holding_name, percentage=pct, isin=holding_isin
                        )
                    )
        ...
        if expand_allocations:
            ...
            countries_xml = await _fetch_ajax_data(
                client, isin, "holdingsSection-countries-loadMoreCountries", "id6"
            )
            if countries_xml:
        ...
        if expand_allocations:
            # Try AJAX for full list
            sectors_xml = await _fetch_ajax_data(
                client, isin, "holdingsSection-sectors-loadMoreSectors", "id7"
            )
            if sectors_xml:
        ...
        gettex = None
        if include_gettex:
            gettex = await load_live_quote(isin)
        ...
        return EtfOverview(
            ...
            != "currency unhedged",
            volatility_1y=_parse_percentage(
                basic_data.get("Volatility 1 year (in EUR)")
            ),
